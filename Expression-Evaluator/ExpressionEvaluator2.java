/**
 * Represents an ExpressionEvaluator2
 * @author Nick Feibel
 * @version 1.0
 */
import java.util.*;
public class ExpressionEvaluator2 extends ExpressionEvaluator{
	...
	*/
	...
	/**
	* Evaluates the expression, slightly different than ExpressionEvaluator
	* as this requires variables longer than one character to be interpreted as
	* one variable.
	...
	*/
	public static Double evaluate(String expr) throws RuntimeException{
		...
		//Loop removes any extra spaces in evalBack while placing
		//evalBack's contents in evalFor which has the contents in
		//front to back order
		while(!evalBack.empty()){
			if(!evalBack.peek().equals("")){
	...
	/**
	* Confirms the number for a variable by requesting the user to input value, accounts for multiletter variables.
	...
	* Slightly different from ExpressionEvaluator as this accounts for multiletter variables.
		...
		for(int i = 0; i<exprSplit.length; i++){
			...
			//If it containes the end parentheses, it goes through a separate sort
				...
				for(int k = 0; k<exprSplit[i].length();k++){
					...
					//If it is not a number, it is known to be a variable
					else if(exprSplit[i].substring(k,k+1).matches("[A-Za-z]")){
						eval.push(exprSplit[i].substring(currentFront,k));
						currentNumber = "";
						while(k<exprSplit[i].length()&&exprSplit[i].substring(k,k+1).matches("[A-Za-z]")){				
							currentNumber += exprSplit[i].substring(k,k+1);
							k++;
						}
						currentFront = k;
					...
					//If none of the above, it is an expression that should be pushed
					else{
						eval.push(exprSplit[i].substring(k,k+1));
					}
				}

				//If it is not all ends, an end parentheses is added
				if(!allEnds){
					eval.push(")");
				}
			}
			//If the String starts as a variable, it goes through a sort to confirm the variable
			else if(exprSplit[i].length()>0&&exprSplit[i].substring(0,1).matches("[A-Za-z]")){
				currentFront = 0;
				for(int z = 0; z<exprSplit[i].length();z++){
					if(exprSplit[i].substring(z,z+1).matches("[A-Za-z]")){
						eval.push(exprSplit[i].substring(currentFront,z));
						currentFront = z+1;
						currentNumber = "";
						while(z<exprSplit[i].length()&&exprSplit[i].substring(z,z+1).matches("[A-Za-z]")){
							currentNumber += exprSplit[i].substring(z,z+1);
							z++;
						}
						eval.push(currentNumber);
					}
